...
void init_spi() {
    ...
    spi_device_interface_config_t devcfg = {
        ...
        .duty_cycle_pos = 128,
        .cs_ena_pretrans = 3,
        .cs_ena_posttrans = 3,  // Keep the CS low 3 cycles after transaction, to stop the master from missing the last
                                // bit when CS has less propagation delay than CLK
...
void draw_screen1(void) {
    ...
    sprintf(&data_str[3][0], "%c %d%c%c", state.scrn1->recieved_char[0], state.scrn1->recieved_num,
            state.scrn1->recieved_char[1], state.scrn1->recieved_char[2]);
